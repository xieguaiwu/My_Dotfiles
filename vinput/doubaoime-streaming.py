...
def _credentials_pinned_by_env() -> bool:
    return bool(get_optional_env("VINPUT_ASR_DEVICE_ID") or get_optional_env("VINPUT_ASR_TOKEN"))
...
def handle_server_message(message: bytes, state: SessionState, request_id: str) -> None:
    ...
    # Only commit on nonstream_result (non-stream final path from server),
    # NOT on server-side VAD (is_vad_finished). Server VAD breaks sentences
    # mid-utterance when the speaker pauses. Instead, let the daemon's
    # explicit "finish" event control finalization.
    if nonstream_result:
