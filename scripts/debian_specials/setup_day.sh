...
sudo apt update
sudo apt upgrade -y
sudo apt install -y curl git npm wget gawk nodejs fastfetch wireguard steghide vlc parole timg
...
cd ~/Downloads
wget "https://github.com/sheeki03/tirith/releases/download/v0.1.9/tirith_0.1.9_amd64.deb"
sudo dnf -y install ./tirith_0.1.9_amd64.deb
