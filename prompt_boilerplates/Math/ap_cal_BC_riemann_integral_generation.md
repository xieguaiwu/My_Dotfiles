---
name: ap-calculus-bc-riemann-integral-practice
version: 1.1.0
description: 生成AP Calculus BC黎曼和与定积分转换专项练习(LaTeX格式)
triggers:
  - "黎曼和"
  - "定积分转换"
  - "Riemann sum"
...
8. **选项难度与干扰项设计（重要）**：
   - **每个错误选项必须有其明确意义**：说明该选项对应哪种常见错误（如：使用错误的黎曼和类型、子区间数量计数错误、函数值取值点偏移、Δx 计算错误等）
...
10. **知识边界约束**：本 skill 遵守 [知识边界规范](../knowledge_boundary.md)。黎曼和积分转换和误差分析是模型容易出错的主题。对于求和式到定积分的转换、误差方向判定（凹凸性关系）不确定时，通过具体例子验证而非硬答。干扰项必须有明确计算错误来源，不能凭空编造。
