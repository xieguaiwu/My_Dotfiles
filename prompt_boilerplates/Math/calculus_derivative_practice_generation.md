...
9. **知识边界约束**：本 skill 遵守 [知识边界规范](../knowledge_boundary.md)。求导计算是模型容易出错的内容之一。对于链式法则多层嵌套、隐函数求导、参数方程求导等复杂推导不确定时，优先通过具体数值代入或符号计算验证而非硬答。干扰项必须有明确的计算错误来源（如忘记链式法则、符号错误、系数遗漏），不能编造"看起来合理"但实际无依据的选项。
