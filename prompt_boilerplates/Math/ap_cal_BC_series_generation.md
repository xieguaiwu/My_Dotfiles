...
11. **知识边界约束**：本 skill 遵守 [知识边界规范](../knowledge_boundary.md)。级数收敛性判定和泰勒级数展开是模型最容易出错的内容之一。对于比值判别法计算结果、拉格朗日误差界计算等不确定时，通过具体数值代入验证而非硬答。常见函数的麦克劳林级数必须核对标准形式，不能编造不存在的展开式。干扰项必须有明确的计算或判定错误来源。

12. **Git 安全网 + 文件写入安全**：本 skill 遵守 [Git 安全网规范](../git_safety_net.md)。执行 `write`/`edit` 前必须先读取并执行 `git_safety_net.md` 中的 git 版本追踪指令。同时：使用 `write` 前必须用 `glob` 或 `read` 确认目标文件是否已存在；若文件已存在，优先用 `edit` 追加内容，而非直接 `write` 覆写；确需覆写须先告知用户。
