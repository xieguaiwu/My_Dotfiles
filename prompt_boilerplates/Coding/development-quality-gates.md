...
▶ 关卡 13：用户可见文本   —— 用户会看到的输出/提示符合简短一致规范吗？
```
...
**脚本/程序与用户自动化文字交互的所有输出**（echo、print、Write-Host、日志行、错误提示、交互询问、CLI help）遵守 ASD-STE100（简化技术英语，国际标准）规范：

- 短句：一条消息 ≤ 20 词（中文 ≤ 40 字），一句一个信息
- 指令祈使：提示操作直接说"要做什么"（"Press Enter to continue."），不用"用户应当..."式叙述
- 术语一致：同一程序内同一概念同一措辞（help/error/prompt 一致）
...
### 1.5.0 (2026-08-16)
- 新增：脚本输出文本规范（ASD-STE100）——新增关卡 13「用户可见文本」，脚本/程序对用户的输出（echo、print、Write-Host、日志行、错误提示、交互询问、CLI help）遵守简化技术英语的短句/祈使/术语一致规范，详见 technical-writing-standard.md 第 7 节
