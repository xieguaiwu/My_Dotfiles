...
> **核心原则**：模型输出是截面排序信号（预期窗口 21/42 天），不是逐日点预测。「每日拿昨日预测比较并自动矫正」在数学上不可行（单日 SNR < 0.03，对单日偏差反应 = 对白噪声滤波），但「每日记账 + 滚动结算矫正」可行。**快慢两层分离是唯一正确架构。**

#### 5.1 两层架构（快慢分离）

```text
诊断层（快，每日，只观察+门控，不改模型）
  D1 regime 标签: 市场动量 mom20 > 0? → 仓位档位
  D2 单日相对表现: top-k 收益 vs 全池中位数（只报警）
  D3 执行差: 实盘成交价 vs 模型假设价（台账）

结算层（慢，T+h 结算，驱动矫正）
  S1 realized IC: 当期全体 rank vs realized return
  S2 预期偏差率: realized/predicted（滚动 ≥60 结算窗口）
  S3 regime 分档正确性: 信号日标签 vs 实现超额
```

**数学依据**：21 天标签、IC≈0.1 的信号，有效校准需 ≥20 个结算样本、≥60 个才稳定。单日信息对 21 天预期的贝叶斯更新量 ≈ 1/21。

#### 5.2 矫正动作分级（保守 → 激进）

| 级别 | 动作 | 触发条件（示例，须按项目校准） | 数据支撑要求 |
|:--|:--|:--|:--|
| L0 | 记录（无动作） | 每日 | — |
| L1 | 预期校准显示 | 滚动偏差率 ≠ 1（±50% 内） | 结算层 S2 |
| L2 | 仓位门控 | mom20 < 0 减半；< -5% 空仓观察 | 分档后验差（实测 A股 +0.45pp，下行仍正 → 减仓不空仓） |
| L3 | 公式降级/备份切换 | 滚动 realized IC < 阈值持续 ≥20 结算 | 非重叠结算流；检测窗口 ≤2 月（防 lifecycle 滞后教训） |
| L4 | 重训 | 季度例行 or L3 连续触发 | 训练协议 |

**三条红线**：
- ❌ 单日结果 → 改公式/阈值（F26）
- ❌ 用重叠窗口滚动 IC 判定降级（F28）——必须用非重叠结算流
- ❌ 在同一批结算样本上既校准又评估——留出评估窗口

#### 5.3 生产信号必备字段（预期完整性）

信号输出必须携带**路径波动预期**，否则持有人会在 Day 1 心理崩溃（F26 的行为学根源）：

```yaml
slot1:
  code: sz000828
  expected_h21: {mean: +1.05%, ci95: [+0.28%, +1.88%], p25: -2.5%}
  path_risk: {p_hit_sl: 0.47, median_mdd: -4.2%, hold_median: 35d}   # ← 必备
  regime: {mom20: +5.51%, tier: up, position_cap: 2}                 # ← 必备
  calibrated_expectation: +0.98%   # 原预期 × 滚动偏差率（L1）
```
...
| 纪律 | 说明 | 违反后果（真实案例） |
|:--|:--|:--|
| **单一权威数据源** | 信号/分析只用生产管线机器的数据副本，其余标 `_stale` | 本地主 NPZ 停在 5 周前（僵尸），差点污染全部分析 |
| **分析前打印数据最后日期** | 任何分析脚本第一行输出 NPZ 最后交易日 | 信号滞后 2 天 → 期望损失 0.03-0.18%/期（实测） |
| **跨机同步必须 md5 验证** | 数据同步后逐一 md5 对照 | 服务器间脚本版本漂移第 3 次事故 |
| **备选池也要过后验验证** | backups 是候选池不是保险 | 生产备份 sz001213 交集后验 h42 -1.28% 显著负（实测） |
| **执行差台账** | 每笔实盘记录成交价 vs 模型假设价 | 回测入场假设未经实盘校准，预期系统性偏差未知 |

#### 5.5 组合分散的现实检验

同池候选两两 21d 前向收益相关 r = 0.53-0.70（A股低价股池实测）→ **买 2 只 ≈ 买 1.6 只**，分散是幻觉。风控三件套只有：止损纪律（协议内 sl 触发率 40-50% 是常态，触发≠失败）、固定小仓位、多期平滑评价（看 5 期均值，不看单期）。真分散必须跨风格/跨市场/跨资产（候选池扩池才可能）。
...
### 第八支柱：机制假设工程师踩坑清单（H1-H13）

> 来源: VERSION2.5 Q 批 8 条机制假设全批验证（2026-08-31，双机逐位一致）+ R 批 HRHR 进攻侧转向验证（同日，KB F103/F104）——完整版见 `VERSION2.5/docs/2026-08-31_mechanism_engineer_pitfalls.md`。
> **核心边界（先读）**：本清单约束「怎么死」的**方法论缺陷**（条件共线/口径/符号/判据对象），**不约束**信号方向、幅度、未来新结构——方向是数据说了算的，硬编码方向 = 亲手杀死判活机会。

#### 8.1 十三个坑速查表

| # | 坑 | 一句话规则 | 血案来源 |
|:--|:--|:--|:--|
| H1 | 条件变量与事件定义共线 | 条件因子在事件日的分布若含事件固有属性（如首板日 cold=0）→ 用 shift 版或去该分量 | Q1 低估组仅 31 事件（估 ~1,800） |
| H2 | 事件数口径未声明 | 事件数 = {事件日 ∈ [面板起点+H, 面板终点]}，非池内全事件（差 34-46%） | Q5 459→303 / Q6 1672→964 |
| H3 | 覆盖率 ≈ 单日率×窗口 | 事件聚集时偏差大（45%→19.9%）→ 面板化后实测 `np.isfinite(panel).mean()` | Q7 |
| H4 | 负向因子入组合未取反 | 组合每腿方向必须与判活符号一致，负向因子用 rank(−f) | Q3 −0.0120 vs +0.0034 |
| H5 | expected IC 当判死门槛 | expected IC 只作预注册锚定与披露，判活只看 UP-1/UP-2 统计 | Q2 预期 −0.04~−0.06 实 −0.0153 |
| H6 | 条件拆解只押单侧 | 无条件弱信号的条件拆解必须双向预注册、各自判活、各自披露 | Q4 双向都过（+1.72%/−4.02%） |
| H7 | 方向反转当普通 DEAD | 方向反但幅度达线 → 登记「方向替换」候选，另行预注册；原方向写入 FALSIFICATION | Q6 机构卖方 +1.71%（预期 −） |
| H8 | 事件表符号/日期前缀未核 | 对齐前抽 3-5 条核对 symbol/日期格式；`_to_grid` 后检查 n_obs 与表行数同量级 | Q5/Q6/Q8 |
| H9 | 数据文件键名/维度靠惯例 | 加载前打印 `{k: arr.shape}` 核对（2D vs 3D、498 vs 5004 池、features vs stock_features） | pc/pf2/holder 三处运行时错误 |
| H10 | 组合增量 bootstrap 对象错 | 对「增量 = combo − max(腿)」做 42d 块 bootstrap，禁止对组合 IC 自身做 CI 冒充增量显著 | Q3 初版假阳性 |
| H11 | 并发裁剪=未声明策略自由度 | 事件篮子的仓位/并发上限+选择规则本身能把正事件集变负组合（按低估分位降序=崩溃日挑最深的刀）；预注册必含裁剪方向性对照（升序/随机） | R4 +1.72%→笔均+0.81%（1249/1495 被裁） |
| H12 | 期块等权与事件等权可反号 | 高风险事件时间聚集于崩溃日→两种聚合口径可反号（笔均+0.81% vs 块均−0.32%）；主判口径锁前写死、双口径同时披露、绝对/超额双列、p90/maxDD 双尾必报 | R4（「高收益」是尾部事件不是期望） |
| H13 | 特征列时间/NaN 语义未核 | 用前读构建源码：是否滞后列（lu_1d flag@t⇔真板 t-1，板前状态要 shift(2)）；NaN 是否有信息（cold_days >60d=NaN → 最冷股被构造性排除，缺失子集可能就是机制人群） | R3 修正版 n=0 自锁 / Q4 人群限定（KB F104） |

#### 8.2 与既有支柱的分工

- 第八支柱 = 机制假设**设计期**检查单（生成假设时逐条过）
- 第一/四支柱 = 假 alpha 与泄露**验证期**检测（F1-F38）
- 第七支柱 = 物理因子**生产路线**（八步全链）；第八支柱与其互补（通用设计坑 vs 推导流程）
- 方向/幅度发现类结果落 `FALSIFICATION_SUMMARY.md` + `mechanism_hypotheses.json`；不硬编码为模型约束
...
| A股 828 Day1 -0.82% 误判失效 | 2026-08-18 | 深市 -1.09% 跟跌=beta；21 天预期被当逐日点预测 | F26 |
| A股盐田港反向信号 | 2026-08-18 | 低波腿×高位置单独触发，h21 -1.8%（位置分档差 4.4pp） | F27 |
| A股重叠口径 +3.5% vs 非重叠 +1.3% | 2026-08-18 | 重叠持仓参照当预期，乐观 2.7× | F28 |
...
### 1.7.0 (2026-08-31 晚)
- **第八支柱扩充 H1-H10 → H1-H13**（来源 R 批 HRHR 会话，KB F103/F104）：H11 并发裁剪=未声明策略自由度（事件篮子选择规则可把正事件集变负组合，预注册必含裁剪方向性对照）/ H12 期块等权与事件等权可反号（事件时间聚集；主判口径锁前写死+双口径双列双尾披露）/ H13 特征列时间语义与 NaN 语义必核（lu_1d 滞后标志⇔板前状态需 shift(2)；cold_days NaN=构造性排除最冷股，缺失子集可能就是机制人群）
- 与 H1/H3/H9 分工：H9 核形状（键名/维度），H13 核语义（滞后/NaN 信息性）；H11/H12 是事件篮子组合层专属（H4/H10 是连续因子组合层）
- 同步：踩坑指南 v1.1.0（VERSION2.5/docs/2026-08-31_mechanism_engineer_pitfalls.md）+ Coding/index.md 2.0.7

### 1.6.0 (2026-08-31)
- **新增第八支柱「机制假设工程师踩坑清单（H1-H10）」**：条件变量与事件共线（H1）/ 事件数口径（H2）/ 覆盖率实测（H3）/ 负向因子组合取反（H4）/ expected IC 非判活门槛（H5）/ 双向预注册（H6）/ 方向反转登记（H7）/ 事件表前缀核验（H8）/ 数据键名维度核验（H9）/ 组合增量 bootstrap 对象（H10）——完整版见 `VERSION2.5/docs/2026-08-31_mechanism_engineer_pitfalls.md`
- **核心边界声明**：本支柱约束方法论缺陷（死因结构），不约束信号方向/幅度/未来新结构（方向由数据决定）
- 来源：VERSION2.5 Q 批 8 条假设全批验证（2026-08-31，双机逐位一致，Q4/Q5 判活 + Q6 方向反转 + 4 DEAD + 1 组合不显著）
- front matter 新增 triggers：因子假设 / 预注册 / 条件拆解
...
- **新增第五支柱「实盘校准——预测记账与结算闭环」**（来自 VERSION2.5 首轮实盘准备的全部验证）：
  - 两层架构（§5.1）：诊断层每日只观察+门控；结算层 T+h 结算驱动矫正——「每日自动矫正公式」数学上不可行（单日 SNR<0.03），「每日记账+滚动结算矫正」可行（校准需 ≥20 结算样本）
  - 矫正动作分级 L0-L4（§5.2）：记录→预期校准→仓位门控→公式降级→重训，附三条红线
  - 生产信号必备字段（§5.3）：路径波动预期（p_hit_sl/中位最大回撤）+ regime 标签——防 Day 1 心理误判
  - 实盘数据纪律（§5.4）：单一权威源/打印数据日期/md5 同步/备选池验证/执行差台账
  - 分散现实检验（§5.5）：同池 r>0.5 → 组合≈单票，风控靠止损+仓位+多期平滑
- **新增 F26-F28 实盘阶段假 alpha 模式**：单日噪声响应过拟合、公式腿单独触发反向信号、重叠口径参照偏差
- 总模式数：25 → 28
- 生产改进注册表与校准闭环设计全文：`VERSION2.5/docs/2026-08-18_prediction_calibration_design.md`
