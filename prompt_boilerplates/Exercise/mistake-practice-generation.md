...
**理科（物理/化学/数学等）——禁止堆砌计算量：**
- 升级方式：换更隐蔽的子类型（如牛顿定律：平衡力 → 作用反作用对 → 多物体系统）、加推理链（每步简单，中间量需自己定义）、加边界情况（无摩擦/理想条件/参数=0）、干扰项改为对应概念误解
- 数据修改 ×1.5-3x 只用于防机械记忆，**不用于制造难度**——升级难度靠维度，不靠把数字改大

**文科（SAT R&W 等）——允许句法复杂度，禁止信息重复：**
- 升级方式：句子加长必须伴随结构层次（嵌套/插入/长距依赖/多重否定）；语义甄别加深（干扰项从"含明显语法错"→"全部语法正确仅语义区分"）；推理层级提升（定位 → 推断 → 评价）
- 禁止：生僻词堆砌、故意晦涩、超纲背景知识

**通用规则：**
1. 同家族内每题只升级一个维度（学生能定位自己错在哪一层）
2. Hard 题最多组合 2 个维度
3. Hard 题干扰项 = 概念错误/语义混淆，不是"算错"或"明显语法错"
4. Hard 题答案必须可验证，解析含完整推理链
5. 反堆料自检：难题的计算量/句长 ≤ 简单题的 3 倍？超过 → 砍量、换维度
...
### 难度升级（v1.2.0 强制）
- [ ] 升级按维度而非堆料：理科计算量 ≤ 简单题 3 倍；文科长句结构层次而非信息重复
- [ ] 每题只升级一个维度（Hard ≤ 2 个组合）
- [ ] Hard 题干扰项对应概念错误（理科）/全部语法正确仅语义区分（文科）
- [ ] Hard 题答案可验证，解析含完整推理链
- [ ] 答案文件每题解析标注考察维度与能力目标
