...
1. **知识边界约束**: 本 skill 遵守 [知识边界规范](../knowledge_boundary.md)。**解题时最容易发生模型幻觉的环节是对不确定的题目类型或内容进行猜测性解答**。如果截图内容不清晰、题目信息不完整、或涉及超出模型可靠判断范围的专业领域，明确告知用户不确定性而非编造答案。在使用信息提取 agent 后仍无法确认题目内容时，应请求用户确认而非继续猜测解答。

2. **准确性优先**: 确保题目内容和符号转录准确无误
3. **公式规范**: 数学公式必须使用正确的 LaTeX 语法
