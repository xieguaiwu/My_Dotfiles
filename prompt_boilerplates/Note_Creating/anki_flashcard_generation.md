...
本 skill 遵守 [Git 安全网规范](../git_safety_net.md)。执行所有 `write` 操作前，必须先读取并执行 `git_safety_net.md` 中的 git 版本追踪指令。

同时遵守以下基本写入安全规则：
1. **写入前先检查**：使用 `glob` 或 `read` 确认目标 `.apkg` 文件是否已存在
2. **`write` 仅用于新建**：确保目标文件确实不存在再使用 `write`
3. **覆写前确认**：如果必须覆写已有文件，先告知用户并获得明确许可
