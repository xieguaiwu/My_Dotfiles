...
export class Editor {
    ...
    async runAutocompleteRequest(requestId, controller, snapshotText, snapshotLine, snapshotCol, options) {
        ...
        // Auto-accept when there's exactly 1 item and the prefix matches it
        // exactly (user typed the full completion). Also handles the Tab case.
        const exactMatchIndex = this.getBestAutocompleteMatchIndex(suggestions.items, suggestions.prefix);
    ...
    updateAutocomplete() {
        if (!this.autocompleteState || !this.autocompleteProvider)
            return;
        // Always use force: false for typing updates — force mode is only for
        // explicit Tab-triggered requests. Persisting force from Tab to typing
        // causes slash-command autocomplete to be skipped, making the dropdown
        // disappear when the user types the exact command name.
