...
import { discoverAgents, discoverAgentsAll, findAgentConfig, type ChainConfig } from "../agents/agents.ts";
...
const parseAgentArgs = (
	...
	for (const step of steps) {
		if (!findAgentConfig(agents, step.name)) {
...
export function registerSlashCommands(
	...
	pi.registerCommand("run", {
		...
		handler: async (args, ctx) => {
			...
			const agents = discoverAgents(state.baseCwd, "both").agents;
			if (!findAgentConfig(agents, agentName)) { ctx.ui.notify(`Unknown agent: ${agentName}`, "error"); return; }
