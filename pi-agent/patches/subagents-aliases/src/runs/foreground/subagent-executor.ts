...
import { findAgentConfig, type AgentConfig, type AgentScope } from "../../agents/agents.ts";
...
async function resumeAsyncRun(input: {
	...
	const agents = intercomBridge.active
		...
		: discoveredAgents;
	const agentConfig = findAgentConfig(agents, target.agent);
	if (!agentConfig) {
		return {
...
function validateExecutionInput(
	...
	if (hasSingle && params.agent && !findAgentConfig(agents, params.agent)) {
		return {
	...
	if (hasTasks && params.tasks) {
		for (let i = 0; i < params.tasks.length; i++) {
			...
			if (!findAgentConfig(agents, task.agent)) {
				return {
	...
	if (hasChain && params.chain) {
		...
		for (let i = 0; i < params.chain.length; i++) {
			...
			for (const agentName of stepAgents) {
				if (!findAgentConfig(agents, agentName)) {
					return {
...
function runAsyncPath(data: ExecutionContextData, deps: ExecutorDeps): AgentToolResult<Details> | null {
	...
	if (hasTasks && params.tasks) {
		const agentConfigs = params.tasks.map((task) => findAgentConfig(agents, task.agent));
	...
	if (hasSingle) {
		const a = findAgentConfig(agents, params.agent);
		if (!a) {
			return {
				content: [{ type: "text", text: `Unknown agent: ${params.agent}` }],
				isError: true,
...
async function runForegroundParallelTasks(input: ForegroundParallelRunInput): Promise<SingleResult[]> {
	return mapConcurrent(input.tasks, input.concurrencyLimit, async (task, index) => {
		...
		const agentConfig = input.findAgentConfig(agents, task.agent);
...
async function runParallelPath(data: ExecutionContextData, deps: ExecutorDeps): Promise<AgentToolResult<Details>> {
	...
	for (const t of tasks) {
		const config = findAgentConfig(agents, t.agent);
		if (!config) {
			return {
...
async function runSinglePath(data: ExecutionContextData, deps: ExecutorDeps): Promise<AgentToolResult<Details>> {
	...
	const allProgress: AgentProgress[] = [];
	const allArtifactPaths: ArtifactPaths[] = [];
	const agentConfig = findAgentConfig(agents, params.agent);
	if (!agentConfig) {
		return {
			content: [{ type: "text", text: `Unknown agent: ${params.agent}` }],
			isError: true,
